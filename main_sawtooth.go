...
func main() {
	...
	for i := 0; i < MAX_THREAD_NUM; i++ {
		...
		curr = (blackestIndex + OFFSET) % PIN_NUMBER
		pinSequence = append(pinSequence, curr)
	}
	...
	for i, index := range pinSequence {
		if i%2 == 0 && i+1 != len(pinSequence) {
			coord1, coord2 := PinCoord[index], PinCoord[pinSequence[i+1]]
			dc.DrawLine(coord1.X, coord1.Y, coord2.X, coord2.Y)
		}
	}
	dc.Stroke()

	dc.SavePNG("result_sawtooth.png")
}
