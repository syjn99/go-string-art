				...
				for _, v := range min_distance {
					...
					for _, v := range blackness_delta {
						...
						for _, v := range line_width {
							...
							calculatePins(PinCoord, PIN_NUMBER)
							...
							for i := 0; i < MAX_THREAD_NUM; i++ {
								...
								for j := 0; j < PIN_NUMBER; j++ {
									if isDistant(curr, j, MIN_DISTANCE, PIN_NUMBER) {
										if usedLines[curr][j] || usedLines[j][curr] {
											continue
										}
										lineBlackness = 0
										trace := getLineTrace(curr, j, PinCoord)
								...
								trace := getLineTrace(curr, blackestIndex, PinCoord)
								for _, coord := range trace {
...
func calculatePins(PinCoord []Coordinate, PIN_NUMBER int) {
...
func getLineTrace(i, j int, PinCoord []Coordinate) []Coordinate {
...
func isDistant(i, j, MIN_DISTANCE, PIN_NUMBER int) bool {
